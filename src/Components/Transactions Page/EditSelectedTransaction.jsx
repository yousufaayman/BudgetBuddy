...
export const EditSelectedTransactions = ({
  ...
  const [inputValues, setInputValues] = useState({
    ...
    date: editSelectedTransaction?.data?.date
      ? formatDate(editSelectedTransaction.data.date)
      : "",
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:3002/user/updateTransaction/${user}/${walletId}/${editSelectedTransaction.id}`,
        {
          title,
          amount,
          category,
  ...
  const handleDelete = async () => {
    try {
      const response = await axios.delete(
        `http://localhost:3002/user/deleteTransaction/${user}/${walletId}/${editSelectedTransaction.id}`,
        {},
      );
